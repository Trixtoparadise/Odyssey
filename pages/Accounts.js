...
const Accounts = ({navigation}) => {
    ...
    const Item = ({item}) => {
      ...
      if (!searchText || searchText.trim().length < 1) {
       return (
        <AccountInfo key={item.Account_number} Account={accInfo} ToWhere={() => navigation.navigate('ViewAccount', {Account_holder_Id: item.Account_holder_Id, Account_holder: " " + item.First_name + " " + item.Last_name, Account_number: item.Account_number, Account_Type: item.Account_Type, Balance: item.Balance})}/>
       );
      }
      ...
      if (indx < 0) return null;
       ...
       return (
        <AccountInfo key={item.Account_number} Account={leftText + keyWord + rightText} ToWhere={() => navigation.navigate('ViewAccount', {Account_holder_Id: item.Account_holder_Id, Account_holder: " " + item.First_name + " " + item.Last_name, Account_number: item.Account_number, Account_Type: item.Account_Type, Balance: item.Balance})}/>
       );
   };
